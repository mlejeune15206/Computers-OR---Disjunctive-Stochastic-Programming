...
param d_k{i in I,k in K}:= f_k[k,i]-s[i]+1; #duration of i in k
...
set Tik{i in I,k in K}:= {t in T: f_k[k,i] >= t >= s[i]};
set Itk{t in T,k in K}:= {i in I: f_k[k,i] >= t >= s[i]};
...
sum{t in a..r} sum{i in Ctk[t,k]} y[k,t,i]*tilde_xi[k,i,t]-r*F >= R * beta[k,r] + (1-beta[k,r])*M[k,r]; 
...
#subject to 6e_DEC{k in K_test, t in T, i in Itk[t,k], kk in #Stk[k,i,t]}: 
#y[k,t,i] >= y[kk,t,i];
...
##########################################
########################################
subject to STREN_DEC{k in K_test}:
(beta[k,r])^2 >= beta[k,r];
